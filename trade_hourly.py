...
def fallback_rule(exec_: UpbitExec) -> int:
    ...
    last = df.iloc[-1]
    if last["ma_fast"] > last["ma_slow"]: return 2
    if last["ma_fast"] < last["ma_slow"]: return 0
    return 1
...
def main(loop=True):
    ...
    while True:
        ...
        try:
            ...
            print("데이터 준비 실패:", e); continue

        action = decide(model, obs) if model is not None else fallback_rule(ex)
        if action == 2:
            ok, resp = ex.buy_fraction(); print("BUY:", ok, resp)
        elif action == 0:
            ok, resp = ex.sell_fraction(); print("SELL:", ok, resp)
        else:
            print("HOLD")
